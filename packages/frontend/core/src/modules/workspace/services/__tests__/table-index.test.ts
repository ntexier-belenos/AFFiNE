...
describe('TableIndexService', () => {
  ...
  describe('getAllTables', () => {
    ...
    it('should return all registered tables', () => {
      ...
      expect(tables.map((t: any) => t.id)).toContain(tableId1);
      expect(tables.map((t: any) => t.id)).toContain(tableId2);
    });
  });
  ...
  describe('getAvailableTablesForRelations', () => {
    it('should return all tables when no exclusion', () => {
      ...
      expect(available.map((t: any) => t.id)).toContain(tableId1);
      expect(available.map((t: any) => t.id)).toContain(tableId2);
    });
